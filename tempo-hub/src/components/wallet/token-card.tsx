...
export function TokenCard({ token }: TokenCardProps) {
  const gradientClass = TOKEN_COLORS[token.symbol] || 'from-gray-500/20 to-gray-600/10 border-gray-500/30'
  ...
  return (
    <Card className={`bg-gradient-to-br ${gradientClass} border overflow-hidden transition-all hover:scale-[1.02] hover:shadow-lg`}>
      <CardContent className="p-5">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={`flex items-center justify-center w-10 h-10 rounded-full bg-card/50 ${iconColor}`}>
